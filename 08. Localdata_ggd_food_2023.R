...
table(is.na(foodshop$open_date))#결측치확인
#na값 제외
foodshop <- foodshop %>%
  filter(open_date != '') %>%
  select(name,type,status,open_date,close_date,address)
...
#5.address변수
foodshop$district<-substr(foodshop$address,4,7)
#시 정보를 분리하여 변수 생성
table(foodshop$district)#이상치 확인
foodshop$district <- ifelse(foodshop$district%in%c(",106","6번지","도 밀양","도 영암", "별시 강","별시 관","별시 금","별시 노","별시 마","별시 용","별시 은","사회","역시 계","역시 남","역시 미","역시 서"),NA,foodshop$district)
#이상치제거
...
foodshop %>%
  ...
  #결측치제거, 영업데이터 추출
  filter(type%in%c("횟집","정종/대포집/소주방","분식","일식","중국식","외국음식전문점(인도,태국등)"))%>%
  group_by(type) %>%
  #업종별 분류
  filter(open_date==min(open_date)) %>% 
  #개업일이 가장 빠른 데이터 추출
